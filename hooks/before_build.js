        ...
        try {
            if (ctx.opts.platforms.indexOf('ios') >= 0) {
                ...
                setTimeout(function() {
                    ...
                    console.log("Ì†æÌ¥≠ copying delegate AppDelegateHeader file from " + srcFile_h + " to " + destFile_h);
                    console.log("Ì†æÌ¥´ copying delegate AppDelegateImp file from " + srcFile_m + " to " + destFile_m);
                }, 0);
            }
        } catch (e) {
            console.log("‚ùå ", e);
        }
    } else throw new Error("Ì†ΩÌ∏± Apple Wallet plugin can not find the directory 'platforms/ios', please try to add ios platform first, then build!")
